...
const { User, Trip, Destination, Document, DocumentType } = require("../models");
...
router.post("/trip", async (req, res) => {
    try {
        ...
        if (tripData) {
            ...
            res.status(500).json({ message: "There was an error while creating the trip" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
router.put("/trip/:id", async (req, res) => {
    try {
        ...
        if (tripData) {
            ...
            res.status(500).json({ message: "There was an error while updating the trip" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
